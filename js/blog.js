document.addEventListener('DOMContentLoaded', () => {
        ...
        const postContent = document.getElementById('post-content');
                              ...
                              // --- Utility: Get Sanity image URL from ref ---
                              function sanityImageUrl(ref) {
                                          if (!ref) return '';
                                          // ref looks like: image-e4aaa8f4ff0d91664ad3e5a613ef358a546f17ea-1200x1200-png
            const parts = ref.split('-');
                                          const format = parts[parts.length - 1];
                                          const dimensions = parts[parts.length - 2];
                                          const id = parts.slice(1, parts.length - 2).join('-');
                                          return `https://cdn.sanity.io/images/9ksnhows/production/${id}-${dimensions}.${format}`;
                              }
            ...
            blocks.forEach((block, index) => {
                            if (block._type !== 'block') return;
                                ...
                                // Peek ahead to see if list ends
            ...
            let bodyHtml = '';
                                          if (Array.isArray(post.body)) {
                                                          ...
                                                          bodyHtml = '<p><em>Content formatting error.</em></p>';
            ...
            postContent.innerHTML = imageHtml + bodyHtml;
                              ...
                              async function fetchPosts() {
                                          try {
                                                          // Sanity GROQ query
                                              const query = encodeURIComponent('*[_type == "post"] | order(date desc) { title, "slug": slug.current, date, author, summary, body, image }');
                                              ...
                                              if (result.result) {
                                                              ...
                                                              // Check if we should deep link to a post
                                                          ...
                                                          blogGrid.innerHTML = '<p style="text-align: center; width: 100%; grid-column: 1 / -1; color: var(--color-error);">Unable to load blog posts. Please try again later.</p>';
                                          }
                              }
